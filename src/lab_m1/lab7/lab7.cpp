...
void Lab7::Update(float deltaTimeSeconds)
{
    ...
    // Render ground
    {
        ...
        RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, glm::vec3(0.502, 0.502, 0.502));
    }
...
void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    GLint lightMatrix = glGetUniformLocation(shader->program, "light_position");
    glUniform3fv(lightMatrix, 1, glm::value_ptr(lightPosition));

    glm::vec3 eyePosition = GetSceneCamera()->m_transform->GetWorldPosition();
    // TODO(student): Set eye position (camera position) uniform
    GLint eyeMatrix = glGetUniformLocation(shader->program, "eye_position");
    glUniform3fv(eyeMatrix, 1, glm::value_ptr(eyePosition));

    // TODO(student): Set material property uniforms (shininess, kd, ks, object color)
    GLint shininessMatrix = glGetUniformLocation(shader->program, "material_shininess");
    glUniform1i(shininessMatrix, materialShininess);

    GLint kdMatrix = glGetUniformLocation(shader->program, "material_kd");
    glUniform1f(kdMatrix, materialKd);

    GLint ksMatrix = glGetUniformLocation(shader->program, "material_ks");
    glUniform1f(ksMatrix, materialKs);

    GLint colorMatrix = glGetUniformLocation(shader->program, "object_color");
    glUniform3fv(colorMatrix, 1, glm::value_ptr(color));
